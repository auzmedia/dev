...
const langData = {
    ...
    kg: {
        ...
        service_2_title: "Каabarlar jana Bot Avtomatlaştıruu",
        service_2_desc: "Instagram API аркылуу автоматтык жоопторду жөнөтүү, сайтты Telegram ботко туташтыруу, табыштамаларды түз Google Таблицага жөнөтүү.",
        ...
        form_label_phone: "Телефон номериңez",
        form_label_package: "Сизге кайсы пакет ылайыктуу?",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function changeLanguage(lang) {
        if (!lang) return;
        lang = lang.toLowerCase().trim(); // Harf xatolarini tozalash
    ...
    if (langSelect) {
        // 1. Xotirani tekshirish. Agar bo'sh bo'lsa qat'iy 'ru' (Rus tili) qo'yiladi
        let savedLang = localStorage.getItem('selectedLang');
        
        if (!savedLang || !langData[savedLang.toLowerCase().trim()]) {
            savedLang = 'ru';
        ...
        // 2. Select dropdown qiymatini o'rnatish
        langSelect.value = savedLang;
        
        // 3. Kafolatlash uchun HTML option ichidan ham 'selected' atributini yoqish
        const activeOption = langSelect.querySelector(`option[value="${savedLang}"]`);
        if (activeOption) {
            activeOption.selected = true;
        }

        // 4. Matnlarni tarjima qilish
        changeLanguage(savedLang);
        console.log("Hozirgi yuklangan til: ", savedLang); // Konsolda tekshirish uchun

        // 5. Foydalanuvchi qo'lda o'zgartirganda ishlaydigan hodisa
        langSelect.addEventListener('change', (e) => {
            ...
            console.log("Til o'zgartirildi va saqlandi: ", chosenLang);
        });
    }
});
